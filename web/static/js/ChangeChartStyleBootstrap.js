...
// Written 11/01/12setDialBackgroundColor
...
//***************************************************
var DialEnd = 1;
var NumOfDials = new Array(1,2,2,3,4,4,6,5,6,8,2,3,4,1);
...
for(var i=0; i< 16; i++)
	{
		document.getElementById("PGNDialID").options[i].disabled = true;
		document.getElementById("PGNDialID").options[i].style.color = '#C0C0C0';
		document.getElementById("DialID").options[i].disabled = true;
		document.getElementById("DialID").options[i].style.color = '#C0C0C0';
	}
	
for(var i=0; i< DialEnd; i++)
	{
		document.getElementById("PGNDialID").options[i].disabled = false;
		document.getElementById("PGNDialID").options[i].style.color = '#0D27C3';
		document.getElementById("DialID").options[i].disabled = false;
		document.getElementById("DialID").options[i].style.color = '#0D27C3';
	}	

createChartArea(PageLayout);
              ...
              {   
                 // N2KValues =  GetPGNbyNumber(String(mydata), DialPGNInstance[DialIndex], DialPGNNumber[DialIndex], DialPGNParameter[DialIndex], DialUnits[DialIndex] );
				  N2KValues =  GetPGNbyNumber(mydata, DialPGNInstance[DialIndex], DialPGNNumber[DialIndex], DialPGNParameter[DialIndex], DialUnits[DialIndex] );
				   UpdateChartValues(CanvasIndex++, N2KValues);
              }
...
//
function UpdateChartValues(DialIndex,  myNewDialValue)
{

  // console.log(mydata[0].description);


	myJSONData[parseInt(DialIndex)]= myNewDialValue.pop();

	if (typeof chart[parseInt(DialIndex)] !== 'undefined' && chart[parseInt(DialIndex)] !== null) {	

	 chart[parseInt(DialIndex)].setTitle(null, { text: 'value = ' + myJSONData[parseInt(DialIndex)]});
	//  chart[parseInt(DialIndex)].title.text = 'mytest';
  // alert(myJSONData[parseInt(DialIndex)]);
...
myDialID = parseInt(myCurrentCanvas);
...
document.getElementById("DialID").options[myDialID].disabled = false;
document.getElementById("DialID").options[myDialID].style.color = '#0D27C3';

document.getElementById("PGNDialID").selectedIndex = myDialID;
document.getElementById("PGNDialID").options[myDialID].disabled = false;
document.getElementById("PGNDialID").options[myDialID].style.color = '#0D27C3';
//alert('show DialTimeout ' + DialTimeout);
//document.getElementById("UpdateInterval").value = UpdateInterval/1000;
//document.getElementById("PGNTimeout").value = DialTimeout;
...
for(i=0; i< document.getElementById("OverlayScale").options.length; i++)
{
		if( document.getElementById("OverlayScale").options[i].value ==  DialAlarm[myDialID])
		document.getElementById("OverlayScale").selectedIndex = i;
...
//document.getElementById("AlarmState").value = DialAlarmState[myDialID];
  ...
  //  document.getElementById("PGNNumber").selectedIndex = DialPGNNumber[myDialID];

/*
  for(i=0; i< document.getElementById("PGNNumber").options.length; i++)
  {
	    SeriesKey = document.getElementById("PGNNumber").options[i].value;
		 if(SeriesKey == DialPGNNumber[myDialID])
		 {
		    	document.getElementById("PGNNumber").selectedIndex = i;
				document.getElementById("PGNSource").selectedIndex  =  i;
 				document.getElementById("PGNInstance").selectedIndex  =  i; 
				document.getElementById("PGNType").selectedIndex  =  i;
				document.getElementById("PGNParameter").selectedIndex  =  i; 
		}
	}
*/
...
//	document.getElementById("PGNInstance").selectedIndex = DialPGNInstance[myDialID];
	...
	var myPGNEvent = GetPGNTypeFromSeries(DialPGNNumber[myDialID]);
		 
		  document.getElementById("PGNTypeID").selectedIndex = 0;
		for(i=0; i< document.getElementById("PGNTypeID").options.length; i++)
		{
			var myEvent = document.getElementById("PGNTypeID").options[i].value;
		
			
			  if(myEvent.trim() == myPGNEvent.trim())
		 ...
		 try{
			 var myEventKey  = DialPGNNumber[myDialID];
		 ...
		 if(myValue == ".*")
			...
			document.getElementById("PGNTypeSource").selectedIndex = parseInt(myValue,16) + 1;
...
//	DialTimeout = document.getElementById("PGNTimeout").value
	//alert('Chart total time =  ' + ((UpdateInterval * DialTimeout)/60000) + ' minutes');
//	document.getElementById("TotalChartTime").innerHTML ='Chart total time =  ' + ((UpdateInterval * DialTimeout)/60000) + ' minutes';
}

function setUpdateInterval(sel) {

//	UpdateInterval = (sel.options[sel.selectedIndex].value) * 1000;
//	UpdateInterval = document.getElementById("UpdateInterval").value * 1000;
	//alert('Chart total time =  ' + ((UpdateInterval * DialTimeout)/60000) + ' minutes');
	//document.getElementById("TotalChartTime").innerHTML ='Chart total time =  ' + ((UpdateInterval * DialTimeout)/60000) + ' minutes';
}
...
function setDialID(sel) {
  ...
  //  document.getElementById("PGNInstance").selectedIndex = DialPGNInstance[CurrentDialID];
	...
	for(i=0; i< document.getElementById("OverlayScale").options.length; i++)
	{
			if( document.getElementById("OverlayScale").options[i].value ==  DialAlarm[CurrentDialID])
			document.getElementById("OverlayScale").selectedIndex = i;

	}
	
	
		var myPGNEvent = GetPGNTypeFromSeries(DialPGNNumber[CurrentDialID]);
		...
		for(i=0; i< document.getElementById("PGNTypeID").options.length; i++)
		{
		  ...
		  try{
			 var myEventKey  = DialPGNNumber[CurrentDialID];
			var myPGNArray = myEventKey.split("."); 
...
var myDialID = document.getElementById("DialID").selectedIndex ;
	...
	if (typeof chart[parseInt(CurrentDialID)] !== 'undefined' && chart[parseInt(CurrentDialID)] !== null) {	
	chart[parseInt(CurrentDialID)].setTitle({ text: myText  });
	}
	
	updateShowDataDetailsList();
...
//<option value="27">Amps</option>       
	...
	//alert(DialID);
	if (typeof chart[parseInt(DialID)] !== 'undefined' && chart[parseInt(DialID)] !== null) {

	switch (parseInt(UnitsID)) 
	{
        case 0:
			//radialDial[DialID].setUnitString("Fahrenheit");
			chart[DialID].yAxis[0].setTitle({ text: "Fahrenheit" });
		break;

		case 1:
			//radialDial[DialID].setUnitString("Celsius");
			chart[DialID].yAxis[0].setTitle({ text: "Celsius" });
		break;

		case 2:
			//radialDial[DialID].setUnitString("Kelvin");
			chart[DialID].yAxis[0].setTitle({ text: "Kelvin" });
		break;

		case 4:
			//radialDial[DialID].setUnitString("Knots");
			chart[DialID].yAxis[0].setTitle({ text: "Knots" });
		break;
		
		case 5:
			//radialDial[DialID].setUnitString("MPH");
			chart[DialID].yAxis[0].setTitle({ text: "MPH" });
		break;

		case 6:
			//radialDial[DialID].setUnitString("KPH");
			chart[DialID].yAxis[0].setTitle({ text: "KPH" });
		break;
		
		case 6:
			//radialDial[DialID].setUnitString("KPH");
			chart[DialID].yAxis[0].setTitle({ text: "mps" });
		break;


		case 8:
			//radialDial[DialID].setUnitString("PSI");
			chart[DialID].yAxis[0].setTitle({ text: "PSI" });
		break;

		case 9:
			//radialDial[DialID].setUnitString("KPASCAL");
			chart[DialID].yAxis[0].setTitle({ text: "KPASCAL" });
		break;

		case 10:
			//radialDial[DialID].setUnitString("INHG");
			chart[DialID].yAxis[0].setTitle({ text: "INHG" });
		break;

		case 12:
			//radialDial[DialID].setUnitString("TRUE");
			chart[DialID].yAxis[0].setTitle({ text: "TRUE" });
		break;

		case 13:
			//radialDial[DialID].setUnitString("MAGNETIC");
			chart[DialID].yAxis[0].setTitle({ text: "MAGNETIC" });
		break;

		case 16:
			//radialDial[DialID].setUnitString("DEGREES");
			chart[DialID].yAxis[0].setTitle({ text: "DEGREES" });
		break;
		
		case 17:
			//radialDial[DialID].setUnitString("Radians");
			chart[DialID].yAxis[0].setTitle({ text: "Radians" });
		break;

		case 18:
			//radialDial[DialID].setUnitString("DEGREES");
			chart[DialID].yAxis[0].setTitle({ text: "GALLONS/HR" });
		break;
		
		case 19:
			//radialDial[DialID].setUnitString("Radians");
			chart[DialID].yAxis[0].setTitle({ text: "LITERS/HR" });
		break;

		case 20:
			//radialDial[DialID].setUnitString("Liters");
			chart[DialID].yAxis[0].setTitle({ text: "Liters" });
		break;

		case 21:
			//radialDial[DialID].setUnitString("Gallons");
			chart[DialID].yAxis[0].setTitle({ text: "Gallons" });
		break;

		case 22:
			//radialDial[DialID].setUnitString("CubicMeter");
			chart[DialID].yAxis[0].setTitle({ text: "CubicMeter" });
		break;
		
		case 23:
			//radialDial[DialID].setUnitString("CubicMeter");
			chart[DialID].yAxis[0].setTitle({ text: "Hertz" });
		break;

		case 24:
			//radialDial[DialID].setUnitString("RPM");
			chart[DialID].yAxis[0].setTitle({ text: "RPM" });
		break;

		case 25:
			//radialDial[DialID].setUnitString("RPS");
			chart[DialID].yAxis[0].setTitle({ text: "RPS" });
		break;

		case 26:
			//radialDial[DialID].setUnitString("%");
			chart[DialID].yAxis[0].setTitle({ text: "%" });
		break;

		case 27:
			//radialDial[DialID].setUnitString("Volts");
			chart[DialID].yAxis[0].setTitle({ text: "Volts" });
		break;

		case 28:
			//radialDial[DialID].setUnitString("Amps");
			chart[DialID].yAxis[0].setTitle({ text: "Amps" });
		break;
		
		case 29:
			//radialDial[DialID].setUnitString("Amps");
			chart[DialID].yAxis[0].setTitle({ text: "Watts" });
		break;

		case 30:
			//radialDial[DialID].setUnitString("Meters");
			chart[DialID].yAxis[0].setTitle({ text: "Watts/hr" });
		break;

		case 31:
			//radialDial[DialID].setUnitString("Feet");
			chart[DialID].yAxis[0].setTitle({ text: "kWatts/hr" });
		break;

		case 32:
			//radialDial[DialID].setUnitString("Fathoms");
			chart[DialID].yAxis[0].setTitle({ text: "Feet" });
		break;
		
		case 33:
			//radialDial[DialID].setUnitString("Fathoms");
			chart[DialID].yAxis[0].setTitle({ text: "Meters" });
		break;

		case 34:
			//radialDial[DialID].setUnitString("Indicator");
			chart[DialID].yAxis[0].setTitle({ text: "Mile" });
		break;

		case 35:
			//radialDial[DialID].setUnitString("Switch");
			chart[DialID].yAxis[0].setTitle({ text: "NAUTI MI" });
		break;
		
		case 35:
			//radialDial[DialID].setUnitString("Switch");
			chart[DialID].yAxis[0].setTitle({ text: "Fathoms" });
		break;
		
		case 37:
			//radialDial[DialID].setUnitString("undefined");
			chart[DialID].yAxis[0].setTitle({ text: "Time" });
		break;
		
		case 38:
			//radialDial[DialID].setUnitString("undefined");
			chart[DialID].yAxis[0].setTitle({ text: "Date" });
		break;
		
		case 39:
			//radialDial[DialID].setUnitString("undefined");
			chart[DialID].yAxis[0].setTitle({ text: "Hours" });
		break;
		
		case 40:
			//radialDial[DialID].setUnitString("undefined");
			chart[DialID].yAxis[0].setTitle({ text: "Indicator" });
		break;
		
		case 41:
			//radialDial[DialID].setUnitString("undefined");
			chart[DialID].yAxis[0].setTitle({ text: "Switch" });
		break;
		
		case 42:
			//radialDial[DialID].setUnitString("undefined");
			chart[DialID].yAxis[0].setTitle({ text: "Dimmer Value" });
		break;
		
		case 43:
			//radialDial[DialID].setUnitString("undefined");
			chart[DialID].yAxis[0].setTitle({ text: "Dimmer Status" });
		break;
		...
		case 46:
			//radialDial[DialID].setUnitString("undefined");
			chart[DialID].yAxis[0].setTitle({ text: "millimeters" });
		break;
		
		case 47:
			//radialDial[DialID].setUnitString("undefined");
			chart[DialID].yAxis[0].setTitle({ text: "inches" });
		break;	
		
		
		case 48:
			//radialDial[DialID].setUnitString("undefined");
			chart[DialID].yAxis[0].setTitle({ text: "undefined" });
		break;
...
CreateChartTypes(CurrentCanvasID, parseInt(myDialID), sel.options[sel.selectedIndex].value);

}

function CreateChartTypes(myCanvasIndex, myDialID, myDialType) {
var myDialCanvas;
...
if (typeof chart[parseInt(myDialID)] !== 'undefined' && chart[parseInt(myDialID)] !== null) {

	chart[parseInt(myDialID)].setTitle({ text: DialText[myDialID]  });

	chart[parseInt(myDialID)].series[0].update({ name: DialText[myDialID]   });
	SetDialUnits(myDialID,  DialUnits[myDialID])

		if( DialMin[myDialID] != DialMax[myDialID] )
		{
			chart[parseInt(myDialID)].yAxis[0].setExtremes(parseInt(DialMin[myDialID]), parseInt(DialMax[myDialID]));
		}

		if( DialFrameType[0] == 15)
		{
...
//	alert(sel);
			...
			case 0:
				//chart[parseInt(CurrentDialID)].plotBackgroundColor: '#FCFFC5';
...
var myDialID;
	...
	//	if(typeof(radialDial[myDialID]) != 'undefined' && radialDial[myDialID] != null)
...
function DeleteCookie(name) {alert('delete cookie');
    var exp = new Date();
...
*/
	...
	//alert(CookiePrefix +'Account' + AccountDataFolder + ":" + AccountType + ":" + ConfigOptions +  exp); 
...
exp.setTime(exp.getTime() + (30 * 24 * 60 * 60 * 1000));
 ...
 // alert('ConfigOptions ' + ConfigOptions);
