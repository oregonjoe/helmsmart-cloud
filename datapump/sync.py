...
def convert_influxdbcloud_gpsjson(mytime, valuelat, valuelng, key):
  ...
  try:
    ...
    tagpairs = key.split(".")
    #log.info('freeboard: convert_influxdbcloud_gpsjson tagpairs %s:  ', tagpairs)
    ...
    #log.info('freeboard: convert_influxdbcloud_gpsjson tagpairs %s:  ', myjsonkeys)

    #values = {'value':value}
    #values = {tag5[1]:value}
    values = {'lat':valuelat, 'lng':valuelng}
    ...
    #log.info('freeboard: convert_influxdbcloud_gpsjson %s:  ', ifluxjson)
    ...
    if debug_all: log.info('Sync: NameError in convert_influxdbcloud_gpsjson %s:  ' % str(e))
    
  except:
    if debug_all: log.info('Sync: Error convert_influxdbcloud_gpsjson %s:', mytime)

    e = sys.exc_info()[0]
    if debug_all: log.info("Sync.py Error in convert_influxdbcloud_gpsjson: %s" % e)
...
def insert_influxdb_cloud(fact_info, device, records):
  ...
  try:
    ...
    if debug_all: log.info('Sync:  insert_influxdb write device=%s  points = %s', record[DEVICE], len(mydataIDBC))
    
    #insert into InfluxDB3 short term storage
    insert_influxdb3_cloud(record[DEVICE], mydataIDBC)

    if debug_all: log.info('Sync:  insert_influxdb3_cloud write device=%s  points = %s', record[DEVICE], len(mydataIDBC))
  ...
  if debug_all: log.info("completed inserted into influxDB %s", device)            
